...
**Correction, 2026-09-25, later cycle:** the "Interpretation" section below claims "whichever section gets the *larger* `nu_sub` ends up with *lower* generated H2 than the other." This is wrong, caught by ChatGPT's independent arithmetic check (`comms/FromChatGPTToClaude.md` Round 34) and verified here directly against both raw JSON files, paired by seed: giving a section the stronger dosage (0.02 vs 0.01) *increases* that section's own H2, paired by seed across the two attempts: A's H2 is +0.0028 bits higher on average when A holds 0.02 than when A holds 0.01; B's H2 is +0.0035 bits higher on average when B holds 0.02 than when B holds 0.01 — both consistently positive across all 5 seeds. The actual reason both assignments come out wrong-signed is a baseline asymmetry, not a dosage-direction effect: B's generated H2 is higher than A's in every seed under *both* assignments (compare the two tables above: A_H2 < B_H2 in all 10 rows across both attempts). This baseline gap dominates the small, correctly-signed-but-tiny dosage effect (a section given *more* dosage does gain a little H2, the intuitively expected direction) — it just isn't large enough, or applied to the right section by a wide enough margin within this two-value grid, to overcome the baseline and flip the pooled sign. The original numeric results (-0.0178 / -0.0240 bits, both wrong-signed) are unaffected — only the causal explanation is corrected. The "What this closes"/"exhausted" conclusion below still holds for the reason actually established (baseline dominates; two-value grid insufficient), just not for the reason originally given.
