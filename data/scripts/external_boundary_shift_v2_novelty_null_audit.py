#!/usr/bin/env python3
"""Execute the preregistered boundary-shift-v2 novelty constructive null (Claude's design).

Implements data/external/boundary-shift-v2-novelty-null-manifest-v1.json
exactly as frozen. Identical to boundary-shift-novelty-null (PR #34) except
the split-acceptance rule: prefers a split where EXACTLY ONE of the two
resulting pieces is a new type (never both), falling back to "at least one
new" and then to "unmodified" only when no better split exists. Motivated
by reading the order-share scoring code directly: requiring both pieces new
creates two adjacent tokens that both collapse to the evaluation metric's
<other> bucket, manufacturing spurious order.

Usage:
    python data/scripts/external_boundary_shift_v2_novelty_null_audit.py <units_repo> [--pilot]
"""
...
OUT_JSON = ROOT / "data" / "derived" / "external-boundary-shift-v2-novelty-null-audit-summary.json"
MANIFEST_PATH = ROOT / "data" / "external" / "boundary-shift-v2-novelty-null-manifest-v1.json"
...
def apply_boundary_shift_v2(coupled_tokens: list[str], nu: float, rng: random.Random) -> list[str]:
    result: list[str] = []
    emitted: set[str] = set()
    i = 0
    ...
    while i < n:
        ...
        if eligible and rng.random() < nu:
            ...
            found = False
            # pass 1: prefer exactly one piece new
            for p in positions:
                ...
                if left_new != right_new:
                    ...
                    break
            if not found:
                # pass 2: fall back to at least one piece new
                for p in positions:
                    left, right = combined[:p], combined[p:]
...
def transform_replicate(atomic_tokens: list[str], beta: float, nu: float, seed: int) -> list[str]:
    ...
    shifted = apply_boundary_shift_v2(coupled, nu, rng)
    return shifted
...
def pilot(units_repo: Path, log, nu_values):
    ...
    for nu in nu_values:
        ...
        for i in range(3):
            transformed = transform_replicate(atomic_cache[i], beta=0.0, nu=nu, seed=pilot_post_seeds[i])
        ...
        log(f"pilot nu={nu}: hapax mean={mean_h:.4f} vals={[round(v,4) for v in hapax_vals]} | "
            f"H2 mean={mean_h2:.4f} | order mean={mean_order:.4f} vals={[round(v,4) for v in order_vals]}")
...
def run(units_repo: Path, log, nu: float) -> dict:
    ...
    def run_config(name: str, beta: float, nu_: float, n: int):
        ...
        for i in range(n):
            t = time.time()
            atomic = get_atomic(i)
            transformed_atomic = transform_replicate(atomic, beta, nu_, post_seeds[i])
    ...
    run_config("boundary_shift_v2_only", 0.0, nu, 20)
    run_config("weaker_novelty", 0.5, round(nu * 0.75, 4), 5)
    run_config("stronger_novelty", 0.5, round(nu * 1.25, 4), 5)
    ...
    shift_novelty_rows = replicates["boundary_shift_v2_only"]
    ...
    order_diff = aggregates["boundary_shift_v2_only"]["token_order_share"]["mean"]
    log(f"order diagnostic: boundary_shift_v2_only mean order share = {order_diff:.4f} "
        f"(compare to v1's 0.0227 at nu=1.0)")
    ...
    return {
        "source": source,
        "protocol": "solo design, self-reviewed (logs/2026-09-21-claude-boundary-shift-v2-selfreview.md)",
        "manifest": "data/external/boundary-shift-v2-novelty-null-manifest-v1.json",
        "nu_calibrated": nu,
