#!/usr/bin/env python3
"""Execute the preregistered budget-capped novelty constructive null (Claude's design).

Implements data/external/budget-capped-novelty-null-manifest-v1.json exactly
as frozen. Identical base mechanism and bigram-conditional search to
bigram-novelty-null, but replaces the per-event probability `nu` with a hard
total event budget `B`: the first B eligible repeats (in stream order)
receive a substitution, and the rest of the stream is left untouched
regardless of how many further repeats occur.

Usage:
    python data/scripts/external_budget_capped_novelty_null_audit.py <units_repo> [--pilot]
"""
...
OUT_JSON = ROOT / "data" / "derived" / "external-budget-capped-novelty-null-audit-summary.json"
MANIFEST_PATH = ROOT / "data" / "external" / "budget-capped-novelty-null-manifest-v1.json"
...
def transform_replicate(atomic_tokens: list[str], beta: float, budget: int, seed: int) -> tuple[list[str], int, int]:
    ...
    for i, token in enumerate(atomic_tokens):
        ...
        is_eligible = candidate_str in emitted and len(candidate) >= 2
        if is_eligible:
            eligible_repeats_seen += 1
        if is_eligible and events_used < budget:
            n = len(candidate)
    ...
    return output, events_used, eligible_repeats_seen
...
def pilot(units_repo: Path, log, budgets):
    ...
    for b in budgets:
        hapax_vals, h2_vals, events_vals, eligible_vals = [], [], [], []
        for i in range(3):
            transformed, events_used, eligible = transform_replicate(atomic_cache[i], beta=0.0, budget=b, seed=pilot_post_seeds[i])
            ...
            events_vals.append(events_used)
            eligible_vals.append(eligible)
        mean_h = sum(hapax_vals) / len(hapax_vals)
        mean_h2 = sum(h2_vals) / len(h2_vals)
        log(f"pilot B={b}: hapax={hapax_vals} mean={mean_h:.4f} | H2 mean={mean_h2:.4f} | "
            f"events_used={events_vals} | eligible_repeats_seen={eligible_vals}")


def run(units_repo: Path, log, budget: int) -> dict:
    source, naibbe_module, headlines, scale, modules, targets, cipher, letters, args = setup(units_repo, log)
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    ...
    event_stats: dict[str, dict[str, dict]] = {}

    def run_config(name: str, beta: float, b: int, n: int):
        bucket = replicates.setdefault(name, {})
        stat_bucket = event_stats.setdefault(name, {})
        for i in range(n):
            t = time.time()
            atomic = get_atomic(i)
            transformed_atomic, events_used, eligible = transform_replicate(atomic, beta, b, post_seeds[i])
            ...
            stat_bucket[str(cipher_seeds[i])] = {"events_used": events_used, "eligible_repeats_seen": eligible}
            log(f"{name} seed_index={i}: all_six_pass={evaluation['all_six_pass']} "
                f"events={events_used}/{eligible} ({time.time()-t:.1f}s)")

    run_config("primary", 0.5, budget, 20)
    run_config("budget_novelty_only", 0.0, budget, 20)
    run_config("half_budget", 0.5, budget // 2, 5)
    run_config("double_budget", 0.5, budget * 2, 5)
    ...
    budget_novelty_rows = replicates["budget_novelty_only"]
    ...
    novelty_increase = sum(
        1 for i in range(n_paired)
        if budget_novelty_rows[str(cipher_seeds[i])]["hapax_share_of_types"]
        > baseline_rows[str(cipher_seeds[i])]["hapax_share_of_types"]
    )
    novelty_criterion_pass = sum(1 for r in budget_novelty_rows.values() if r["criteria_pass"]["hapax"])
    ...
    mean_eligible = sum(s["eligible_repeats_seen"] for s in event_stats["primary"].values()) / len(event_stats["primary"])
    mean_events = sum(s["events_used"] for s in event_stats["primary"].values()) / len(event_stats["primary"])
    implied_rate = mean_events / mean_eligible if mean_eligible else 0.0
    log(f"volume diagnostic: primary mean events_used={mean_events:.1f} / mean eligible_repeats={mean_eligible:.1f} "
        f"= implied rate {implied_rate:.4f} (compare to nu=0.1-0.2 tested previously)")
    ...
    return {
        "source": source,
        "protocol": "solo design, self-reviewed (logs/2026-09-21-claude-budget-capped-novelty-selfreview.md)",
        "manifest": "data/external/budget-capped-novelty-null-manifest-v1.json",
        ...
        "volume_diagnostic": {"mean_events_used": mean_events, "mean_eligible_repeats": mean_eligible,
...
def main() -> None:
    ...
    parser.add_argument("--pilot", action="store_true", help="run self-consistency-only B calibration, no full sweep")
    parser.add_argument("--budget", type=int, default=None, help="frozen B value from pilot, required unless --pilot")
    ...
    if args.pilot:
        pilot(args.units_repo.resolve(), log, budgets=[500, 1000, 2000, 3000, 4000, 6000])
        return

    if args.budget is None:
        print("error: --budget is required for a full run (use --pilot first to calibrate)", file=sys.stderr)
        sys.exit(2)

    summary = run(args.units_repo.resolve(), log, args.budget)
