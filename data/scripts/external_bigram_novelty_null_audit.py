#!/usr/bin/env python3
"""Execute the preregistered bigram-novelty constructive null (Claude's design).

Implements data/external/bigram-novelty-null-manifest-v1.json exactly as
frozen. Identical base mechanism to frequency-novelty-null (Naibbe + boundary
coupling at beta=0.50), but replaces the unigram-frequency substitution rule
with an exhaustive bigram-conditional search over the replicate's own running
order-1 atom-transition distribution. baseline/edge_only are NOT rerun --
reused by reference (same reference file as frequency-novelty-null).

Usage:
    python data/scripts/external_bigram_novelty_null_audit.py <units_repo> [--pilot]
"""
...
OUT_JSON = ROOT / "data" / "derived" / "external-bigram-novelty-null-audit-summary.json"
MANIFEST_PATH = ROOT / "data" / "external" / "bigram-novelty-null-manifest-v1.json"
...
def pilot(units_repo: Path, log, nu_values):
    ...
    for nu in nu_values:
        ...
        log(f"pilot nu={nu}: hapax={hapax_vals} mean={mean_h:.4f} | H2={h2_vals} mean={mean_h2:.4f}")
...
def run(units_repo: Path, log, nu: float) -> dict:
    ...
    run_config("bigram_novelty_only", 0.0, nu, 20)
    ...
    bigram_novelty_rows = replicates["bigram_novelty_only"]
    ...
    novelty_increase = sum(
        1 for i in range(n_paired)
        if bigram_novelty_rows[str(cipher_seeds[i])]["hapax_share_of_types"]
        > baseline_rows[str(cipher_seeds[i])]["hapax_share_of_types"]
    )
    novelty_criterion_pass = sum(1 for r in bigram_novelty_rows.values() if r["criteria_pass"]["hapax"])
    ...
    return {
        "source": source,
        "protocol": "solo design, self-reviewed (logs/2026-09-21-claude-bigram-novelty-selfreview.md)",
        "manifest": "data/external/bigram-novelty-null-manifest-v1.json",
        "nu_calibrated": nu,
