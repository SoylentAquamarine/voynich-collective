#!/usr/bin/env python3
"""Execute the preregistered coupling-v3.1 (isolation-aware beta selection) pilot and primary audit.

Implements data/external/coupling-v3-1-corrected-selection-manifest-v1.json exactly as frozen.
Adapted directly from external_coupling_v3_lower_beta_pilot_and_primary_audit.py (coupling-v3,
INVALID_CONSTRUCTION verdict) with exactly one procedural change: the pilot stage measures and
selects on `edge_only` (coupling alone, nu_shift=0, nu_sub=0) directly, instead of the combined
mechanism's edge gain -- see logs/2026-09-25-claude-coupling-v3-1-selfreview.md for the design
reasoning behind this correction. Everything else (coupling-v2's identity-mapping target rule,
boundary-shift-v2, substitution top-up, six frozen criteria, evaluation code) is unchanged.

Two modes:
  --pilot         run the 3-seed-per-beta edge_only calibration grid from the manifest, report
                   mean isolated edge gain per beta. No selection or freezing happens here --
                   read the printed table and pick the lowest beta clearing the edge_only floor.
  --beta VALUE    run the full 20-seed primary evaluation at the selected beta (combined
                   mechanism), plus a fresh 20-seed edge_only boundary manipulation check at that
                   same beta, plus the H2-headroom comparison against coupling-v2's own primary
                   configuration (hardcoded below, from PR #71's committed summary -- not
                   recomputed, since coupling-v2 itself is unchanged and already verified).

Usage:
    python data/scripts/external_coupling_v3_1_corrected_selection_audit.py <units_repo> --pilot
    python data/scripts/external_coupling_v3_1_corrected_selection_audit.py <units_repo> --beta 0.40
"""
...
OUT_JSON = ROOT / "data" / "derived" / "external-coupling-v3-1-corrected-selection-audit-summary.json"
MANIFEST_PATH = ROOT / "data" / "external" / "coupling-v3-1-corrected-selection-manifest-v1.json"
...
def apply_coupling_v3(atomic_tokens: list[str], beta: float, rng: random.Random) -> list[str]:
    """coupling-v3/v3.1: identical target rule to coupling-v2 (target = prev_last, 26 distinct
...
def pilot(units_repo: Path, log):
    """Corrected pilot: measures edge_only (coupling alone) directly, not the combined mechanism."""
    source, naibbe_module, headlines, scale, modules, targets, cipher, letters, args = setup(units_repo, log)
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    cfg = manifest["coupling_v3_1"]
    ...
    print(f"{'beta':>6} | {'edge_only_mean':>14} | {'positive_blocks_mean':>20} | clears_edge_only_floor")
    results = {}
    for beta in cfg["beta_pilot_grid"]:
        edge_vals, positive_vals = [], []
        for i in range(cfg["pilot_seeds_per_beta"]):
            # edge_only: coupling alone, nu_shift=0, nu_sub=0 -- the exact quantity coupling-v3
            # measured only as part of its later validity check, now measured HERE, at selection time.
            transformed = transform_replicate(
                atomic_cache[i], beta=beta, nu_shift=0.0, nu_sub=0.0,
            ...
            positive_vals.append(ev["edge_positive_blocks"])
        mean_edge = sum(edge_vals) / len(edge_vals)
        mean_positive = sum(positive_vals) / len(positive_vals)
        clears = mean_edge >= SIX_CRITERIA["edge"]["gain_min"]
        results[beta] = {"edge_only_mean": mean_edge, "positive_blocks_mean": mean_positive, "clears_edge_only_floor": clears}
        print(f"{beta:6.2f} | {mean_edge:14.4f} | {mean_positive:20.2f} | {clears}")

    qualifying = [b for b, r in results.items() if r["clears_edge_only_floor"]]
    if qualifying:
        selected = min(qualifying)
        print(f"\nSelected primary beta per manifest rule (lowest qualifying on edge_only alone): {selected}")
    else:
        print("\nNo beta in the pilot grid clears the edge_only floor -- per the manifest's honesty "
              "precommitment, this is a negative result closing off the lower-fixed-beta lever "
              "entirely. Do not expand the grid without a fresh precommitment.")
    return results
...
def run(units_repo: Path, log, beta: float) -> dict:
    ...
    v3_1_headroom = COUPLING_V2_PRIMARY_H2_CEILING - primary_h2_max
    headroom_improved = v3_1_headroom > COUPLING_V2_PRIMARY_H2_HEADROOM_BITS
    ...
    return {
        "source": source,
        "protocol": "solo design, self-reviewed (logs/2026-09-25-claude-coupling-v3-1-selfreview.md)",
        "manifest": "data/external/coupling-v3-1-corrected-selection-manifest-v1.json",
        "selected_beta": beta,
        "nu_sub": nu_sub,
        ...
        "h2_headroom_bits": v3_1_headroom,
...
def main() -> None:
    ...
    parser.add_argument("--pilot", action="store_true", help="run the edge_only beta calibration grid, no selection frozen")
