#!/usr/bin/env python3
"""Execute the preregistered hybrid shift-v2+substitution coupling-v2 novelty null (Claude's design).

Implements data/external/hybrid-shift-coupling-v2-substitution-novelty-null-manifest-v1.json
exactly as frozen. Identical to hybrid-shift-v2-substitution (PR #40) EXCEPT the coupling
mechanism's target-selection rule: instead of TARGET_INITIALS[ATOMIC_ALPHABET.index(prev_last) % 4]
(4 distinct possible targets, causally traced by PR #44 as the source of the order-share
failure), coupling-v2 uses target = prev_last directly (26 distinct possible targets, zero
concentration by construction) -- see logs/2026-09-23-claude-coupling-v2-selfreview.md for the
full design reasoning. Everything else (boundary-shift-v2, substitution top-up, six frozen
criteria, evaluation code) is unchanged from the v1-coupling hybrid script.

IMPORTANT: edge_only must be computed FRESH under coupling-v2, not reused from the v1-coupling
reference -- edge_only measures coupling's own contribution, and the coupling rule has changed.
baseline (beta=0.0) remains safe to reuse by reference, since coupling never fires at beta=0.0
regardless of which target rule is defined.

Usage:
    python data/scripts/external_hybrid_shift_coupling_v2_substitution_novelty_null_audit.py <units_repo> [--pilot]
"""
...
OUT_JSON = ROOT / "data" / "derived" / "external-hybrid-shift-coupling-v2-substitution-novelty-null-audit-summary.json"
MANIFEST_PATH = ROOT / "data" / "external" / "hybrid-shift-coupling-v2-substitution-novelty-null-manifest-v1.json"
...
def apply_coupling_v2(atomic_tokens: list[str], beta: float, rng: random.Random) -> list[str]:
    """coupling-v2: target = prev_last directly (26 distinct possible targets, zero
    concentration by construction), in place of the original 4-way TARGET_INITIALS lookup.
    Trigger probability (beta) and every other mechanic are unchanged from every prior design."""
    output: list[str] = []
    prev_last = None
...
def apply_boundary_shift_v2(coupled_tokens: list[str], nu_shift: float, rng: random.Random) -> list[str]:
    """Unchanged from external_hybrid_shift_v2_substitution_novelty_null_audit.py."""
    result: list[str] = []
    emitted: set[str] = set()
    i = 0
...
def transform_replicate(atomic_tokens: list[str], beta: float, nu_shift: float, nu_sub: float, seed: int) -> list[str]:
    ...
    coupled = apply_coupling_v2(atomic_tokens, beta, rng)
...
def run(units_repo: Path, log, nu_sub: float) -> dict:
    ...
    # NOTE: unlike the v1-coupling hybrid script, only `baseline` is reused by reference here.
    # `edge_only` depends on the coupling rule (it measures coupling's own contribution) and
    # MUST be recomputed fresh under coupling-v2 -- see manifest and selfreview log.
    ...
    run_config("edge_only", 0.5, 0.0, 0.0, 20)
    ...
    manipulation_checks = {
        "boundary": {"paired_edge_increase": edge_increase, "edge_criterion_pass": edge_criterion_pass,
                     "pass": boundary_check_pass, "computed_fresh_under_coupling_v2": True},
    ...
    return {
        "source": source,
        "protocol": "solo design, self-reviewed (logs/2026-09-23-claude-coupling-v2-selfreview.md)",
        "manifest": "data/external/hybrid-shift-coupling-v2-substitution-novelty-null-manifest-v1.json",
