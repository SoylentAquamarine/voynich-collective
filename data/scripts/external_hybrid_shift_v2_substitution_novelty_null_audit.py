#!/usr/bin/env python3
"""Execute the preregistered hybrid shift-v2+substitution novelty null (Claude's design).

Implements data/external/hybrid-shift-v2-substitution-novelty-null-manifest-v1.json
exactly as frozen. Identical to the original hybrid design (PR #35, INVALID_CONSTRUCTION
on token-order-share) EXCEPT the boundary-shift component uses the already-validated v2
split rule (PR #36: prefer a split where exactly one piece is new) instead of the original
v1 rule (both pieces must be new) -- the same fix that already resolved this identical
order-share failure mode in the standalone boundary-shift design, applied here because the
original hybrid was built before that fix existed and was never updated to use it. Everything
else (coupling, substitution top-up, evaluation) is unchanged from the original hybrid script.

Usage:
    python data/scripts/external_hybrid_shift_v2_substitution_novelty_null_audit.py <units_repo> [--pilot]
"""
...
OUT_JSON = ROOT / "data" / "derived" / "external-hybrid-shift-v2-substitution-novelty-null-audit-summary.json"
MANIFEST_PATH = ROOT / "data" / "external" / "hybrid-shift-v2-substitution-novelty-null-manifest-v1.json"
...
def run(units_repo: Path, log, nu_sub: float) -> dict:
    ...
    return {
        "source": source,
        "protocol": "solo design, self-reviewed (logs/2026-09-22-claude-hybrid-shift-v2-substitution-selfreview.md)",
        "manifest": "data/external/hybrid-shift-v2-substitution-novelty-null-manifest-v1.json",
