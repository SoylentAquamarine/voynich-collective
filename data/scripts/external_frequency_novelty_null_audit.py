#!/usr/bin/env python3
"""Execute the preregistered frequency-novelty constructive null (Claude's design).

Implements data/external/frequency-novelty-null-manifest-v1.json exactly as
frozen. Identical base mechanism to boundary-state-null (Naibbe + boundary
coupling at beta=0.50), but replaces the uniform-substitution novelty rule
with an exhaustive frequency-weighted search over the replicate's own running
atom-unigram distribution. baseline/edge_only are NOT rerun -- reused by
reference from the prior boundary-state-null audit per the frozen manifest,
since baseline (beta=nu=0) is seed-invariant by construction (identity
transform) and edge_only (nu=0) is explicitly committed to reuse.

Usage:
    python data/scripts/external_frequency_novelty_null_audit.py <units_repo> [--pilot]
"""
...
OUT_JSON = ROOT / "data" / "derived" / "external-frequency-novelty-null-audit-summary.json"
MANIFEST_PATH = ROOT / "data" / "external" / "frequency-novelty-null-manifest-v1.json"
...
def verify_source(repo: Path, manifest: dict) -> dict:
    # frequency-novelty-null-manifest-v1.json doesn't repeat external_source (identical
    # to boundary-state-null); pull the checksums from the reference file's provenance.
        ...
        if candidate_str in emitted and len(candidate) >= 2 and rng.random() < nu:
            ...
            for pos in positions:
                ...
                if use_cold_start:
                    ...
                    keyed = [(-freq.get(a, 0), rng.random(), a) for a in alternatives]
                    keyed.sort()
...
def pilot(units_repo: Path, log, nu_values):
    ...
    for nu in nu_values:
        ...
        log(f"pilot nu={nu}: hapax={hapax_vals} mean={mean_h:.4f}")
...
def run(units_repo: Path, log, nu: float) -> dict:
    ...
    run_config("frequency_novelty_only", 0.0, nu, 20)
    ...
    freq_novelty_rows = replicates["frequency_novelty_only"]
    ...
    novelty_increase = sum(
        1 for i in range(n_paired)
        if freq_novelty_rows[str(cipher_seeds[i])]["hapax_share_of_types"]
        > baseline_rows[str(cipher_seeds[i])]["hapax_share_of_types"]
    )
    novelty_criterion_pass = sum(1 for r in freq_novelty_rows.values() if r["criteria_pass"]["hapax"])
    ...
    return {
        "source": source,
        "protocol": "solo design, self-reviewed (logs/2026-09-20-claude-frequency-novelty-selfreview.md)",
        "manifest": "data/external/frequency-novelty-null-manifest-v1.json",
        "nu_calibrated": nu,
...
def main() -> None:
    ...
    if args.pilot:
        pilot(args.units_repo.resolve(), log, nu_values=[0.3, 0.45, 0.6, 0.75])
        return
