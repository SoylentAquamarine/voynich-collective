#!/usr/bin/env python3
"""Diagnostic (not a preregistered mechanism test): under coupling-v3.1's own
mechanism (Naibbe + coupling-v2's identity-mapping rule at beta=0.15, fixed +
boundary-shift-v2, nu_shift=1.0, fixed), does varying the substitution top-up
dosage (nu_sub) by real Currier-label section construct any part of the real
pooled Currier A/B character-bigram-entropy gap (+0.278 bits)?

Design reasoning, result-blinding disclosure, and the honesty precommitment
are in logs/2026-09-25-claude-section-aware-coupling-v3-1-selfreview.md,
written before this script ran. Dosage values (nu_sub_A=0.02, nu_sub_B=0.01)
are reused UNCHANGED from coupling-v2's own already-executed sensitivity grid
(0.02 = "stronger_topup", 0.01 = primary) -- not chosen to hit this target.
Which section gets which value is arbitrary and disclosed as such in the
self-review log.

coupling-v3.1's own beta (0.15) and nu_shift (1.0) are held fixed, identical
across both sections -- only nu_sub varies by section, mirroring exactly how
the precedent diagnostic (external_currier_ab_construction_diagnostic.py)
varied only its own single novelty parameter, not its coupling parameter.

Usage:
    python data/scripts/external_coupling_v3_1_section_aware_diagnostic.py <units_repo>
"""
...
OUT_JSON = ROOT / "data" / "derived" / "external-coupling-v3-1-section-aware-diagnostic-summary.json"
...
NU_SUB_B = 0.01   # coupling-v3.1's own (and coupling-v2's own) primary value
NU_SUB_DEFAULT = 0.01
...
def apply_section_varying_substitution_topup(shifted_tokens, token_labels, nu_by_label, rng):
    ...
    global value -- mirrors external_currier_ab_construction_diagnostic.py's
    apply_section_varying_substitution, adapted to this project's own
    substitution-top-up implementation."""
    output: list[str] = []
    emitted: set[str] = set()
...
def main() -> None:
    ...
    log(f"frozen dosages (reused unchanged from coupling-v2's own sensitivity grid): "
    ...
    summary = {
        "base_mechanism": "coupling-v3.1 (beta=0.15 fixed) + boundary-shift-v2 (nu_shift=1.0 fixed) + section-varying substitution top-up",
        ...
        "frozen_dosages": {
            ...
            "provenance": "nu_sub_A/B reused unchanged from coupling-v2's own already-executed sensitivity grid (0.02=stronger_topup, 0.01=primary); beta reused unchanged from coupling-v3.1's own frozen primary; section assignment (A=stronger) is arbitrary, disclosed in the self-review log, not chosen to hit this target",
        },
        "replicates": results,
        "mean_gap": mean_gap,
