#!/usr/bin/env python3
"""Execute the preregistered hybrid shift+substitution novelty null (Claude's design).

Implements data/external/hybrid-shift-substitution-novelty-null-manifest-v1.json
exactly as frozen. Combines two previously-independent mechanisms: boundary-shift
(PR #34, provably H1/H2-invariant but capped near 64% hapax) runs first at its
own established maximum capacity, then a small bigram-conditional substitution
top-up (PR #30's mechanism) closes the remaining hapax gap on whatever tokens
are still repeats afterward.

Usage:
    python data/scripts/external_hybrid_shift_substitution_novelty_null_audit.py <units_repo> [--pilot]
"""
...
OUT_JSON = ROOT / "data" / "derived" / "external-hybrid-shift-substitution-novelty-null-audit-summary.json"
MANIFEST_PATH = ROOT / "data" / "external" / "hybrid-shift-substitution-novelty-null-manifest-v1.json"
...
def apply_boundary_shift(coupled_tokens: list[str], nu_shift: float, rng: random.Random) -> list[str]:
    result: list[str] = []
    emitted: set[str] = set()
    i = 0
...
def transform_replicate(atomic_tokens: list[str], beta: float, nu_shift: float, nu_sub: float, seed: int) -> list[str]:
    ...
    shifted = apply_boundary_shift(coupled, nu_shift, rng)
...
def run(units_repo: Path, log, nu_sub: float) -> dict:
    ...
    return {
        "source": source,
        "protocol": "solo design, self-reviewed (logs/2026-09-21-claude-hybrid-shift-substitution-selfreview.md)",
        "manifest": "data/external/hybrid-shift-substitution-novelty-null-manifest-v1.json",
