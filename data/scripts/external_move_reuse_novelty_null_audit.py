#!/usr/bin/env python3
"""Execute the preregistered move-reuse novelty constructive null (Claude's design).

Implements data/external/move-reuse-novelty-null-manifest-v1.json exactly as
frozen. Identical base mechanism and bigram-conditional fresh-search fallback
to bigram-novelty-null, but caches successful (position, replacement_atom)
moves per token length and prefers reusing one over deriving a fresh move,
testing whether a small repeated repertoire builds learnable substructure
that costs less H2/BPE damage than always-fresh locally-optimal moves.

Usage:
    python data/scripts/external_move_reuse_novelty_null_audit.py <units_repo> [--pilot]
"""
...
OUT_JSON = ROOT / "data" / "derived" / "external-move-reuse-novelty-null-audit-summary.json"
MANIFEST_PATH = ROOT / "data" / "external" / "move-reuse-novelty-null-manifest-v1.json"
...
def transform_replicate(atomic_tokens: list[str], beta: float, nu: float, seed: int) -> tuple[list[str], int, int, int]:
    ...
    move_cache: dict[int, list[list]] = defaultdict(list)  # n -> [[pos, atom, use_count], ...]
    reuse_events = 0
    fresh_events = 0
    ...
    for i, token in enumerate(atomic_tokens):
        ...
        if candidate_str in emitted and len(candidate) >= 2 and rng.random() < nu:
            ...
            entries = move_cache.get(n, [])
            entries_sorted = sorted(entries, key=lambda e: (-e[2], entries.index(e)))
            for entry in entries_sorted:
                pos, atom, _uc = entry
                if pos == 0 or pos >= n:
                    continue
                if candidate[pos] == atom:
                    continue
            ...
            if not applied:
                ...
                for pos in positions:
                    ...
                    if found:
                        break
                if found:
                    move_cache[n].append([pos, candidate_str[pos], 1])
                    fresh_events += 1
    ...
    return output, reuse_events, fresh_events, sum(len(v) for v in move_cache.values())
...
def pilot(units_repo: Path, log, nu_values):
    ...
    for nu in nu_values:
        hapax_vals, h2_vals, reuse_vals, fresh_vals, cachesz_vals = [], [], [], [], []
        for i in range(3):
            transformed, reuse_n, fresh_n, cache_sz = transform_replicate(atomic_cache[i], beta=0.0, nu=nu, seed=pilot_post_seeds[i])
            ...
            reuse_vals.append(reuse_n)
            fresh_vals.append(fresh_n)
            cachesz_vals.append(cache_sz)
        mean_h = sum(hapax_vals) / len(hapax_vals)
        mean_h2 = sum(h2_vals) / len(h2_vals)
        log(f"pilot nu={nu}: hapax={hapax_vals} mean={mean_h:.4f} | H2 mean={mean_h2:.4f} | "
            f"reuse_events={reuse_vals} fresh_events={fresh_vals} cache_size={cachesz_vals}")
...
def run(units_repo: Path, log, nu: float) -> dict:
    ...
    reuse_stats: dict[str, dict[str, dict]] = {}

    def run_config(name: str, beta: float, nu_: float, n: int):
        bucket = replicates.setdefault(name, {})
        stat_bucket = reuse_stats.setdefault(name, {})
        for i in range(n):
            t = time.time()
            atomic = get_atomic(i)
            transformed_atomic, reuse_n, fresh_n, cache_sz = transform_replicate(atomic, beta, nu_, post_seeds[i])
            ...
            stat_bucket[str(cipher_seeds[i])] = {"reuse_events": reuse_n, "fresh_events": fresh_n, "cache_size": cache_sz}
            log(f"{name} seed_index={i}: all_six_pass={evaluation['all_six_pass']} "
                f"reuse={reuse_n} fresh={fresh_n} cache={cache_sz} ({time.time()-t:.1f}s)")

    run_config("primary", 0.5, nu, 20)
    run_config("move_reuse_novelty_only", 0.0, nu, 20)
    ...
    reuse_novelty_rows = replicates["move_reuse_novelty_only"]
    ...
    novelty_increase = sum(
        1 for i in range(n_paired)
        if reuse_novelty_rows[str(cipher_seeds[i])]["hapax_share_of_types"]
        > baseline_rows[str(cipher_seeds[i])]["hapax_share_of_types"]
    )
    novelty_criterion_pass = sum(1 for r in reuse_novelty_rows.values() if r["criteria_pass"]["hapax"])
    ...
    primary_reuse = reuse_stats["primary"]
    mean_reuse = sum(s["reuse_events"] for s in primary_reuse.values()) / len(primary_reuse)
    mean_fresh = sum(s["fresh_events"] for s in primary_reuse.values()) / len(primary_reuse)
    mean_cache = sum(s["cache_size"] for s in primary_reuse.values()) / len(primary_reuse)
    reuse_share = mean_reuse / (mean_reuse + mean_fresh) if (mean_reuse + mean_fresh) else 0.0
    log(f"reuse diagnostic: primary mean reuse_events={mean_reuse:.1f} fresh_events={mean_fresh:.1f} "
        f"cache_size={mean_cache:.1f} reuse_share={reuse_share:.4f}")
    ...
    return {
        "source": source,
        "protocol": "solo design, self-reviewed (logs/2026-09-21-claude-move-reuse-novelty-selfreview.md)",
        "manifest": "data/external/move-reuse-novelty-null-manifest-v1.json",
        "nu_calibrated": nu,
        ...
        "reuse_diagnostic": {"mean_reuse_events": mean_reuse, "mean_fresh_events": mean_fresh,
                              "mean_cache_size": mean_cache, "reuse_share": reuse_share},
