...
- **Voynich is more locally constrained than two matched natural-language baselines** (`data/scripts/language_baselines.py`, `data/derived/language-baselines-report.md`; independently reproduced from scratch by Claude — re-downloaded and re-verified both source corpora, got numerically identical output, see `logs/2026-09-18-skeptic-baseline-audit.md`). Using `1 − H2/H1` (fraction of within-word character uncertainty removed by knowing the previous character) on size-matched 39,020-token samples: Voynich 0.4539, Medieval Latin (Index Thomisticus Treebank) 0.2164, Italian (ISDT) 0.2310. Within-word-shuffled controls collapse all three toward near-zero constraint (Voynich 0.0801, Latin 0.0256, Italian ~0.03), confirming the metric is detecting real ordering structure, not an artifact. The result survives an alpha-only sensitivity check that drops the 708 Voynich tokens carrying transcription markup. It also survives a source-order sensitivity check (`data/scripts/baseline_sampling_sensitivity.py`, `data/derived/baseline-sampling-sensitivity-report.md`): all nine non-overlapping Latin windows and six Italian windows remain far below Voynich, as do 200 sentence-randomized samples per corpus (maxima 0.2137 and 0.2343). **This does not identify a language, cipher, or hoax** — it is compatible with a real constrained writing system, a cipher, *and* a mechanically-generated pseudo-text (e.g. a table-and-grille hoax method would tend to produce exactly this kind of strong local structure). Two Indo-European baselines, corpus/genre dependence, and un-atomized EVA symbols remain real confounds — see Open Questions.
...
- Neither the normalized corpus nor the language-baseline script treats extended-EVA/grouped glyphs as atomic symbols — a principled atomic-EVA tokenization is still an open task that could affect both pass 1 and the baseline comparison.
