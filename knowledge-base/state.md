...
- The language-baseline comparison samples "first N tokens in deterministic train/dev/test order" rather than randomly across documents — risks document-clustering skewing the baseline. Needs variance-across-samples or random cross-document sampling before the constraint-ratio gap is treated as more than a single matched comparison. (Skeptic audit, `logs/2026-09-18-skeptic-baseline-audit.md`.)
- Both language baselines are Indo-European (Latin, Italian). A structurally different language family (e.g. agglutinative or a non-alphabetic-adjacent system) would stress-test the constraint-ratio comparison harder.
